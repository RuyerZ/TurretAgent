...
public class RadiusTurretUpgradeBehavior : TurretUpgradeBase
{
    ...
    public override List<(string, float)> GetUpgrades()
    {
        ...
        return new List<(string, float)>() {
            ("Damage: " + _Turret.AttackDamage.ToString("N1"), cost),
            ("CD: " + _Turret._CoolingTimelReset.ToString("N2"), cost),
    ...
    public override bool Upgrade(int index)
    {
        ...
        {
            case 0:
                _Turret.AttackDamage += 0.5f;
                break;
            case 1:
                _Turret._CoolingTimelReset *= 0.8f;
                if (_Turret._CoolingTimelReset < 0.01f)
                    _Turret._CoolingTimelReset = 0.01f;
                break;
            case 2:
